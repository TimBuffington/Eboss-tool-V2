...
def html(markup: str) -> None:
    """Small helper to safely inject HTML."""
...
.home-grid {{
  ...
  gap: var(--gap);
  max-width: var(--maxw);
  margin: 16px auto 120px; /* bottom space for fixed footer */
  padding: 10px;
}}
...
.logo-wrap {{
  ...
  place-items: center;
  margin-inline: auto;
  /* No fixed height -> prevents the extra vertical space */
}}
...
/* ===== Button area -> grid for tidy alignment ===== */
.actions-grid {{
  display: grid;
...
.message-text:hover {{ box-shadow: 0 0 10px {COLORS['Energy Green']}; }}
...
.radio-wrap > div {{ min-width: 0; }} /* prevent overflow clipping */
...
if st.session_state.page == "Home":
    ...
    # Logo
    html('<div class="g-logo"><div class="logo-wrap">')
    st.image(
        "https://raw.githubusercontent.com/TimBuffington/Eboss-tool-V2/main/assets/logo.png",
        use_container_width=True,  # scales inside .logo-wrap
        output_format="PNG",
    )
    html("</div></div>")
    ...
    html('<h1 class="page-title">EBOSS® Size &amp; Spec Tool</h1>')
    html("</div>")

    # Top actions (3 buttons)
    html('<div class="g-actions section"><div class="actions-grid">')
    ...
    # Message
    html('<div class="g-msg"><div class="message-text">Please Select a Configuration</div></div>')

    # Radios
    html('<div class="g-radios radio-wrap">')
    selected_option = st.radio(
        " ",
        (
            "Select a EBOSS® Model",
            "Use Load Based Suggested EBOSS® Model",
        ),
        horizontal=True,
    )
