...
st.markdown("<p style='text-align: center;'>Please Select a Configuration</p>", unsafe_allow_html=True)
    ...
    try:
        ...
        if st.session_state.selected_option == "Use Load Based Suggested EBOSS® Model":
            ...
            with st.dialog("Recommended EBOSS® Configuration") as dialog:
                ...
                with col2:
                    st.text_input("Max Continuous Load", key="max_continuous_load_input")
                    st.text_input("Max Peak Load", key="max_peak_load_input")
                ...
                if st.button("Launch Tool", key="launch_tool_recommended"):
                    ...
                    st.session_state.page = "Tool Selection"  # Placeholder page
                    dialog.close()
                    st.rerun()
