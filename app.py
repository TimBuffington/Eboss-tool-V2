...
def apply_custom_css():
    ...
    html, body, .stApp {
        ...
        font-size: 1.1rem;
        line-height: 1.6;
        background: url("https://raw.githubusercontent.com/TimBuffington/EBOSS&reg-tool-V2/main/assets/bg.png") no-repeat center center fixed;
    ...
    /* === BUTTONS === */
    .stButton > button, .EBOSS&reg-hero-btn {
        width: 100%;
        max-width: 340px;
        margin: 1rem auto;
...
def show_logo_and_title(title):
    st.markdown(
        '<div class="logo-header"><img src="https://raw.githubusercontent.com/TimBuffington/EBOSS&reg-tool-V2/main/assets/logo.png" alt="Company Logo"></div>',
        unsafe_allow_html=True
    )
...
VALID_MODELS = ["EBOSS&reg 25 kVA", "EBOSS&reg 70 kVA", "EBOSS&reg 125 kVA", "EBOSS&reg 220 kVA", "EBOSS&reg 400 kVA"]

# kVA name -> kVA value
EBOSS&reg_KVA = {
    "EBOSS&reg 25 kVA": 25,
    "EBOSS&reg 70 kVA": 70,
    "EBOSS&reg 125 kVA": 125,
    "EBOSS&reg 220 kVA": 220,
    "EBOSS&reg 400 kVA": 400,
}

# Battery sizes (kWh) for EBOSS&reg®
EBOSS&reg_BATTERY_KWH = {
    "EBOSS&reg 25 kVA": 15,
    "EBOSS&reg 70 kVA": 25,
    "EBOSS&reg 125 kVA": 50,
    "EBOSS&reg 220 kVA": 75,
    "EBOSS&reg 400 kVA": 125,
}
...
EBOSS&reg_KVA = globals().get("EBOSS&reg_KVA", {
    "EBOSS&reg 25 kVA": 25, "EBOSS&reg 70 kVA": 70, "EBOSS&reg 125 kVA": 125, "EBOSS&reg 220 kVA": 220, "EBOSS&reg 400 kVA": 400
})
...
def get_fuel_use(
    ...
    Notes:
      ...
      - EBOSS&reg GPD = runtime_hrs_per_day * GPH
      - Standard Gen GPD = 24 * GPH
    """
    # EBOSS&reg model → kVA; fallback to parsing
    kva = EBOSS&reg_KVA.get(model, _first_int(model))
    ...
    power_module = None
    if EBOSS&reg_type == "Power Module" and pm_gen:
...
def update_fuel_use_in_session(std_kva_choice: int) -> dict:
    ...
    model      = ui.get("model", "EBOSS&reg 25 kVA")
    EBOSS&reg_type = ui.get("EBOSS&reg_type", "Full Hybrid")
    cont_kw    = float(ui.get("cont_kw", 0.0))
    charge_kw  = float(ui.get("charge_rate", 0.0))
    pm_gen     = ui.get("pm_gen")

    # use your existing runtime calc so EBOSS&reg uses actual engine-hours/day
    rt = calculate_runtime_specs(model, EBOSS&reg_type, cont_kw, pm_gen)
    runtime_hrs_per_day = float(rt.get("runtime", 0.0))

    fuel_use = get_fuel_use(
        model=model,
        EBOSS&reg_type=EBOSS&reg_type,
        cont_kw=cont_kw,
        charge_kw=charge_kw,
        pm_gen=pm_gen,
        runtime_hrs_per_day=runtime_hrs_per_day,
        std_kva_choice=int(std_kva_choice),
    )
    st.session_state["fuel_use"] = fuel_use
    return fuel_use

def render_fuel_table_from_session(show_pm: bool) -> None:
    """Render a small table using st.session_state['fuel_use']."""
    fuel = st.session_state.get("fuel_use")
    if not fuel:
        st.info("Fuel metrics not computed yet.")
        return
    fh, pm, sg = fuel["full_hybrid"], fuel["power_module"], fuel["standard_gen"]

    st.markdown("### 🔥 Fuel Use (GPH / GPD / GPW / GPM)")
    hdr = st.columns(4)
    hdr[0].markdown("**Metric**")
    hdr[1].markdown("**EBOSS&reg Full Hybrid**")
    hdr[2].markdown("**EBOSS&reg Power Module**")
...
def calculate_charge_rate(model: str, EBOSS&reg_type: str, pm_gen: str | None) -> float:
    """
    Charge rate is determined ONLY by the EBOSS&reg model + mode.
    PM generator size does NOT change the charge rate; it's validated separately.
    Always clamp to max_charge for safety.
    """
    # Resolve model kVA (handles "EBOSS&reg 125 kVA" and minor key quirks)
    model_kva = EBOSS&reg_KVA.get(model)
    if model_kva is None:
        import re
        m = re.search(r"(\d+)", model or "")
        model_kva = int(m.group(1)) if m else 0

    spec = EBOSS&reg_Specs[model_kva]
    max_charge = float(spec["max_charge"])

    if EBOSS&reg_type == "Full Hybrid":
        charge_rate = float(spec["full_hybrid"])
    else:
        # Power Module
        charge_rate = float(spec["power_module"])

    # Safety: never exceed EBOSS&reg max_charge
    return min(charge_rate, max_charge)

def get_charge_rate(model: str, EBOSS&reg_type: str) -> float:
    """
    Charge rate depends only on model + mode.
    PM generator size does NOT affect the rate; it's validated separately.
    Always clamp to max_charge for safety.
    """
    if model not in EBOSS&reg_KVA:
        raise ValueError(f"Unknown model: {model}")

    kva = EBOSS&reg_KVA[model]              # numeric kVA for the EBOSS&reg model
    spec = EBOSS&reg_Specs[kva]             # envelope for this kVA
    base_rate = spec["power_module"] if EBOSS&reg_type == "Power Module" else spec["full_hybrid"]
...
def validate_pm_generator(model: str, pm_gen: str) -> tuple[bool, list[str]]:
    ...
    pm_charge_rate = float(EBOSS&reg_Specs[model_kva]["power_module"])
...
def enforce_session_validation():
    ...
    model = ui.get("model")
    EBOSS&reg_type = ui.get("EBOSS&reg_type") or "Full Hybrid"
    pm_gen = ui.get("pm_gen")

    # PM validation rules (do NOT change rate, just gate + warn)
    if EBOSS&reg_type == "Power Module":
        if not pm_gen:
...
def landing_page():
    apply_custom_css()
    show_logo_and_title("EBOSS&reg&reg Specs and Comparison Tool")
    ...
    with col2:
        ...
        link_button("🎥 Learn How EBOSS&reg® Works",
...
def validate_charge_rate(model: str,
    ...
    kva = EBOSS&reg_KVA[model]
    spec = EBOSS&reg_Specs[kva]
    ...
    if EBOSS&reg_type == "Power Module" and gen_kw is not None:
        gen_kw = float(gen_kw)
...
def render_user_input_form():
    with st.container():
        ...
        with cols[0]:
            st.markdown('''
                <div class="card">
                    <div class="card-label">EBOSS&reg®</div>
            ''', unsafe_allow_html=True)

            st.session_state.user_inputs["model"] = st.selectbox(
                "Model", list(EBOSS&reg_KVA.keys()), key="model_select"
            )

            st.session_state.user_inputs["EBOSS&reg_type"] = st.selectbox(
                "Type", ["Full Hybrid", "Power Module"], key="EBOSS&reg_type_select"
            )

            if st.session_state.user_inputs["EBOSS&reg_type"] == "Power Module":
...
def display_load_threshold_check(user_inputs):
    ...
    EBOSS&reg_type = user_inputs.get("EBOSS&reg_type")
    cont_kw = user_inputs.get("cont_kw")

    if not model or model not in EBOSS&reg_KVA or model not in EBOSS&reg_BATTERY_KWH:
        st.warning("⚠️ Invalid model selected or missing data.")
        return

    pm_kva = EBOSS&reg_KVA[model]
    battery_kwh = EBOSS&reg_BATTERY_KWH[model]

    try:
        charge_rate = EBOSS&reg_Charge_Rates[pm_kva]["power_module" if EBOSS&reg_type == "Power Module" else "full_hybrid"]
    ...
    if cont_kw > charge_rate:
        ...
        recommended = None
        for name, kva in EBOSS&reg_KVA.items():
            new_rate = EBOSS&reg_Charge_Rates[kva]["power_module" if EBOSS&reg_type == "Power Module" else "full_hybrid"]
            if cont_kw <= new_rate * 0.9:
                recommended = name
                break

        if recommended:
            st.warning(f"💡 Recommended EBOSS&reg size: **{recommended}** for your current load.")
        else:
            st.error("❌ No EBOSS&reg size can handle this load. Consider splitting into multiple units.")
    elif cont_kw > max_safe_limit:
        st.warning(f"⚠️ Load is above 90% of the EBOSS&reg charge rate ({charge_rate:.1f} kW).")
...
def calculate_runtime_specs(model, EBOSS&reg_type, cont_kw, pm_gen):
    # Determine generator size based on mode
    gen_kva = EBOSS&reg_KVA.get(model, 0) if EBOSS&reg_type == "Full Hybrid" else int(__import__("re").search(r"(\d+)", str(pm_gen or "")).group(1)) if pm_gen else 0
    gen_kw = gen_kva * 0.8

    # Use the unified envelope for charge/battery
    model_kva = EBOSS&reg_KVA.get(model, gen_kva)
    charge_kw = calculate_charge_rate(model, EBOSS&reg_type, pm_gen)
    battery_kwh = EBOSS&reg_Specs[model_kva]["battery_kwh"]  # ← unified & correct
...
def render_compare_page():
    import re
    apply_custom_css()
    show_logo_and_title("Compare — EBOSS&reg® vs Standard Generator")
    top_navbar()

    # Selected EBOSS&reg model
    model = st.session_state.user_inputs.get("model_select")
    ...
    EBOSS&reg_specs = spec_data.get(model, {})
    ...
    charge_kw = EBOSS&reg_Specs[kva]["full_hybrid"]
    battery_kwh = EBOSS&reg_Specs[kva]["battery_kwh"]
    ...
    # Fuel estimates (EBOSS&reg runs only during charge; Std Gen runs 24h @ ~100%)
    EBOSS&reg_gph = interpolate_gph(kva, min(max(charge_kw / gen_kw, 0.25), 1.0))
    std_gph   = interpolate_gph(kva, 1.0)

    EBOSS&reg_gpd = round(EBOSS&reg_gph * runtime_hrs, 2)
    std_gpd   = round(std_gph * 24.0, 2)
    EBOSS&reg_gpw, std_gpw = round(EBOSS&reg_gpd * 7, 2), round(std_gpd * 7, 2)
    EBOSS&reg_gpm, std_gpm = round(EBOSS&reg_gpd * 30, 2), round(std_gpd * 30, 2)

    # ---- Sections ----
    sections = {
        "Maximum Intermittent Load": EBOSS&reg_specs.get("Maximum Intermittent Load", []),
        "Maximum Continuous Load":   EBOSS&reg_specs.get("Maximum Continuous Load",   []),
    }

    # Render tech sections (for each metric show EBOSS&reg vs Std Gen as a two-card row)
    for section, rows in sections.items():
        ...
        for label, EBOSS&reg_val in rows:
            ...
            """, unsafe_allow_html=True)
            # then the two cards: EBOSS&reg vs Std Gen
            col1, col2 = st.columns(2, gap="large")
            with col1:
                st.markdown(f"""
                    <div class="card">
                        <div class="card-label">EBOSS&reg®</div>
                        <div class="card-value">{EBOSS&reg_val}</div>
                    </div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
    ...
    metrics = [
        ...
        ("Gallons per Day",     f"{EBOSS&reg_gpd} gal",    f"{std_gpd} gal"),
        ("Gallons per Week",    f"{EBOSS&reg_gpw} gal",    f"{std_gpw} gal"),
        ("Gallons per Month",   f"{EBOSS&reg_gpm} gal",    f"{std_gpm} gal"),
    ]
    for label, eb_val, st_val in metrics:
        ...
        """, unsafe_allow_html=True)
        # EBOSS&reg vs Std Gen as two cards
        col1, col2 = st.columns(2, gap="large")
        with col1:
            st.markdown(f"""
                <div class="card">
...
def render_spec_value_row_4(spec_label: str, EBOSS&reg_val: str, std_val: str, diff_val: str | None = None):
    """Four cards in one row: Spec | EBOSS&reg | Std Gen | Difference (if numeric)."""
    c1, c2, c3, c4 = st.columns(4, gap="large")

    # auto-compute pretty delta unless caller supplies one
    diff_html = _delta_badge(EBOSS&reg_val, std_val) if diff_val is None else _delta_badge(diff_val, "0")
...
def render_load_specs_page():
    ...
    kva = EBOSS&reg_KVA[inputs["model"]]
    spec = EBOSS&reg_Specs[kva]
    ...
    if peak_kw > spec["max_peak"]:
        st.error(f"❌ Peak load ({peak_kw:.1f} kW) exceeds EBOSS&reg peak limit ({spec['max_peak']} kW).")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    # Default the standard-gen to the same kVA as the EBOSS&reg model for convenience
    default_kva = EBOSS&reg_KVA.get(inputs["model"], 125)
    std_kva_choice = st.selectbox(
        "Standard Generator Size (kVA)",
        [25, 45, 65, 125, 220, 400],
        index=[25,45,65,125,220,400].index(default_kva) if default_kva in [25,45,65,125,220,400] else 3,
        key="std_kva_for_load_specs"
    )
    update_fuel_use_in_session(std_kva_choice)
    render_fuel_table_from_session(show_pm=(inputs["EBOSS&reg_type"] == "Power Module" and inputs.get("pm_gen") is not None))
    ...
    """, unsafe_allow_html=True)
    render_spec_value_row("EBOSS&reg® Model", inputs["model"])
    ...
    render_spec_value_row("Max Peak (EBOSS&reg®)", f"{spec['max_peak']:.1f} kW")
...
def render_compare_page():
    ...
    # Selected EBOSS&reg model (normalize the 125 key if needed)
    ...
    """, unsafe_allow_html=True)
    e_rows = EBOSS&reg_specs.get("Maximum Intermittent Load", [])
    ...
    """, unsafe_allow_html=True)
    e_rows = EBOSS&reg_specs.get("Maximum Continuous Load", [])
    ...
    # Compute EBOSS&reg duty-cycle metrics from your envelopes (same method you used before)
    inputs = st.session_state.user_inputs
    ...
    # --- Fuel comparison block (stores dict in session) ---
    ui = st.session_state.user_inputs
    default_kva = int(kva_key) if str(kva_key).isdigit() else EBOSS&reg_KVA.get(ui["model"], 125)
    std_kva_choice = st.selectbox(
        "Standard Generator Size (kVA)",
        [25, 45, 65, 125, 220, 400],
        index=[25,45,65,125,220,400].index(default_kva) if default_kva in [25,45,65,125,220,400] else 3,
    ...
    update_fuel_use_in_session(std_kva_choice)
    render_fuel_table_from_session(show_pm=(ui["EBOSS&reg_type"] == "Power Module" and ui.get("pm_gen") is not None))
...
def render_cost_analysis_page():
    ...
    EBOSS&reg_type = inputs.get("EBOSS&reg_type")
    ...
    EBOSS&reg_rent = st.number_input("EBOSS&reg® Monthly Rental ($)", 0.0, 100000.0, 3800.0, 50.0)
    ...
    if st.button("✅ Run Cost Comparison"):
        runtime = calculate_runtime_specs(model, EBOSS&reg_type, cont_kw, pm_gen)
        ...
        e_co2 = e_fuel * 22.4
        s_co2 = s_fuel * 22.4
        e_total = EBOSS&reg_rent + e_cost + delivery_fee + e_pm_cost
        s_total = std_rent + s_cost + delivery_fee + s_pm_cost
        diff = s_total - e_total

        # ---- COST TABLE ----
        rows = [
            ("Generator Size", f"{EBOSS&reg_KVA[model]} kVA / {int(EBOSS&reg_KVA[model]*0.8)} kW", std_gen, ""),
            ("Rental Cost ($)", EBOSS&reg_rent, std_rent, std_rent - EBOSS&reg_rent),
        ...
        <div class="card" style="text-align:center;">
            <div style="font-size:1.3rem;font-weight:bold;margin-bottom:1rem;">
                EBOSS&reg® Cost Analysis Report
            </div>
            <div style="font-size:0.9rem;">{today}</div>
            <button class="EBOSS&reg-hero-btn" onclick="window.print()" style="margin-top:1.5rem;">
                 🖨️ Print Cost Report
            </button>
        </div>
        ''', unsafe_allow_html=True)
...
def render_parallel_calculator_page():
    ...
    view_mode = st.selectbox("View Output As", ["Equipment Only", "Comparison EBOSS&reg vs Gen-Only"])
    ...
    if st.button("Calculate"):
        ...
        for section, rec in results.items():
            ...
            headers = ["Scenario","EBOSS&reg QTY","Generator QTY","Charge kW","Fuel (gal/day)"]
        ...
        if view_mode == "Comparison EBOSS&reg vs Gen-Only":
...
def render_parallel_calculator_page():
    ...
    view_mode = st.selectbox("View Output As:", ["Equipment Only", "Comparison: EBOSS&reg vs Generator-Only"])
    ...
    if st.button("🔢 Calculate"):
        ...
        st.markdown(f'''
            <div class="print-logo" style="text-align:center; margin-top:2rem;">
              <img src="https://raw.githubusercontent.com/TimBuffington/EBOSS&reg-tool-V2/main/assets/logo.png" width="240"><br><br>
              <div style="font-size:1.3rem; font-weight:bold;">EBOSS&reg® Parallel Sizing Report</div>
              <div style="font-size:0.9rem;">{today}</div>
            </div>
            <button class="EBOSS&reg-hero-btn" onclick="window.print()" style="margin:2rem auto; display:block;">
                🖨️ Print Report
            </button>
            <style>
            @media print {{
...
def calculate_parallel_sizing(required_cont_kw, required_peak_kw, sizing_mode):
    """
    Determines EBOSS&reg and generator sizing options based on required load and strategy.
    ...
    for kva, spec in EBOSS&reg_Specs.items():
        ...
        est_fuel = units_needed * (charge_rate * 0.015)  # EBOSS&reg fuel formula
        results["Full Hybrid Only"].append({
            "scenario": f"EBOSS&reg {kva} kVA Only",
    ...
    all_kvas = list(EBOSS&reg_Specs.keys())
    combos = list(combinations_with_replacement(all_kvas, 2))  # pairs
    mixed = []
    for combo in combos:
        charge = sum(EBOSS&reg_Specs[k]["full_hybrid"] for k in combo)
    ...
    pm_mixed = []
    for combo in combos:
        charge = sum(EBOSS&reg_Specs[k]["power_module"] for k in combo)
        if charge >= required_cont_kw:
            gens = -(-charge // max(EBOSS&reg_Specs[k]["max_charge"] for k in combo))
...
def render_parallel_results(results, view_mode="Equipment Only"):
    ...
    for category, items in results.items():
        ...
        headers = ["Plan", "EBOSS&reg Units", "Generators", "Charge (kW)", "Fuel (gal/day)"]
...
<div class="footer">
    ANA EBOSS&reg&reg;Spec and Comparison Tool &nbsp; | &nbsp;
